...
(defn convert-trinary-to-decimal  ([trinary-str]
  (if (re-matches #"^[0-2]+" trinary-str) ;Matches against string containing multiple characters but all must be 0, 1 or 2.
    (convert-trinary-to-decimal
      (->> trinary-str
           (map #(Character/getNumericValue %));code to convert string to lazy sequence of numbers code is found here
           (reverse))
...
(defn -main []
  ...
  (println(convert-rna-sequence-to-amino-acids "aaaaaaaaaaaaaaaaaaaaaaaa" ))
  )